...
export default function FeatureChecklist() {
  ...
  return (
    <section className="py-28 px-4 sm:px-6 lg:px-8 bg-white text-[#1A1A1A] relative border-t border-zinc-200">
      <div className="max-w-[1366px] mx-auto" ref={containerRef}>
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-[#1A1A1A] font-sans max-w-3xl">
            Engineered To Be Your{" "}
            <span className="font-serif italic font-normal text-[#00B87D]">
              Growth Benchmark.
            </span>
          </h2>
          <p className="text-base sm:text-lg text-zinc-600 max-w-2xl mt-4 font-normal">
        ...
        {/* 2-Column Expressive Typography Checklist */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {checklistItems.map((item, idx) => (
            <div
              key={idx}
              className="expressive-item p-7 sm:p-8 rounded-3xl bg-[#FAFAFA] border border-zinc-200/80 flex items-start gap-4 hover:border-[#00D28F] hover:shadow-xl transition-all duration-300"
            >
