...
import { Compass, Layers, Cpu, Rocket, Check, Plus, Minus, HelpCircle, MessageSquare } from "lucide-react";
...
export default function ProcessTimeline({ onOpenBooking }: ProcessTimelineProps) {
  ...
  return (
    <section id="process" className="py-32 px-4 sm:px-6 lg:px-8 bg-white text-[#1A1A1A] relative border-t border-zinc-200">
      <div className="max-w-[1366px] mx-auto">
        ...
        <div className="mb-32">
          ...
          <div className="flex flex-col items-center text-center mb-20">
            ...
            <h2 className="text-4xl sm:text-6xl font-bold tracking-tight text-[#1A1A1A] font-sans max-w-3xl">
              From Initial Blueprint To{" "}
              <span className="font-serif italic font-normal text-[#00B87D]">
                Market Dominance.
              </span>
            </h2>
            <p className="text-base sm:text-lg text-zinc-600 max-w-xl mt-4 font-normal">
          ...
          <div ref={timelineRef} className="relative max-w-4xl mx-auto">
            ...
            <div className="space-y-12">
              {processSteps.map((step) => (
                <div key={step.number} className="relative flex items-start gap-6 sm:gap-10">
                  {/* Step Node */}
                  <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-white border-2 border-[#00D28F] shadow-md flex items-center justify-center shrink-0 z-10 font-mono font-bold text-[#00A870] text-sm sm:text-base">
                    {step.number}
                  </div>

                  {/* Step Content Card */}
                  <div className="flex-1 p-7 sm:p-8 rounded-3xl bg-[#FAFAFA] border border-zinc-200 shadow-sm hover:border-[#00D28F] hover:shadow-xl transition-all">
                    <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                      <h3 className="text-xl sm:text-2xl font-bold text-[#1A1A1A] font-sans tracking-tight">
                        {step.title}
                      </h3>
                      <span className="px-3 py-1 rounded-full bg-[#00D28F]/15 text-xs font-bold text-[#00A870] uppercase tracking-wider border border-[#00D28F]/30">
                        {step.duration}
                      </span>
                    </div>
        ...
        <div id="faq" className="max-w-4xl mx-auto pt-16 border-t border-zinc-200">
          <div className="flex flex-col items-center text-center mb-16">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-800 text-xs font-bold uppercase tracking-wider mb-4">
              <HelpCircle className="w-3.5 h-3.5 text-[#00B87D]" />
            ...
            {FAQS.map((faq, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={cn(
                    "rounded-2xl border transition-all duration-300 overflow-hidden bg-[#FAFAFA]",
                ...
                >
                  ...
                  >
                    <span className="text-base sm:text-lg font-bold text-[#1A1A1A] tracking-tight font-sans">
                      {faq.question}
                    </span>
                  ...
                  {isOpen && (
                    <div className="px-6 sm:px-8 pb-6 pt-1 text-sm sm:text-base text-zinc-600 font-normal leading-relaxed border-t border-zinc-100">
                      {faq.answer}
                    </div>
                  )}
                </div>
          ...
          {/* Direct Help Card */}
          <div className="mt-12 text-center p-6 sm:p-8 rounded-3xl bg-[#FAFAFA] border border-zinc-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 text-left">
              <div className="w-10 h-10 rounded-2xl bg-[#00D28F]/20 flex items-center justify-center shrink-0">
                <MessageSquare className="w-5 h-5 text-[#00A870]" />
              </div>
              <div>
                <h4 className="text-base font-bold text-[#1A1A1A]">Have a unique requirement?</h4>
                <p className="text-xs text-zinc-500">Speak directly with our technical leadership.</p>
              </div>
            </div>
            <button
              onClick={onOpenBooking}
              className="px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider text-[#0A0A0A] bg-[#00D28F] hover:bg-[#00B87D] transition-all shrink-0 cursor-pointer"
            >
              Ask A Question
            </button>
          </div>
