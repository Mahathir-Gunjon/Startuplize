...
import { Compass, Shield, CheckCircle2, Zap, ArrowUpRight } from "lucide-react";
...
export default function DepthOfVision({ onOpenBooking }: DepthOfVisionProps) {
  ...
  useGSAP(
    () => {
      ...
      if (imageRevealRef.current && imageElementRef.current) {
        ...
        gsap.set(imageElementRef.current, { scale: 1.2 });
      ...
      // 2. Text Content Scrolling Animations with Stagger & 85% Threshold
  ...
  return (
    <section
      ref={sectionRef}
      className="relative min-h-[85vh] overflow-hidden bg-[#FAFAFA] text-[#1A1A1A] py-20 px-4 sm:px-6 lg:px-8 border-b border-zinc-200 flex items-center"
    >
      {/* Background Ambient Radial Glow */}
      <div className="absolute top-1/2 left-1/3 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-emerald-100/50 rounded-full blur-[140px] pointer-events-none -z-10" />
      ...
      <div className="max-w-[1366px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          ...
          >
            ...
            {/* Strictly H2 (text-3xl md:text-5xl) with Instrument Serif Contrast */}
            <h2 className="dov-scroll-anim text-3xl md:text-5xl font-bold tracking-tight text-[#1A1A1A] font-sans leading-[1.1]">
            ...
            <div className="dov-scroll-anim space-y-4 w-full pt-2">
              {[
                ...
                <div
                  key={idx}
                  className="p-4 rounded-xl bg-white border border-zinc-200/90 shadow-xs flex items-start gap-4"
                >
                  ...
                  <div>
                    <h4 className="text-[16px] md:text-[18px] font-bold text-zinc-900 font-sans">{item.title}</h4>
                    <p className="text-[14px] md:text-[16px] text-zinc-500 font-normal mt-1">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>

            {/* Standardized CTA Button Row */}
            <div className="dov-scroll-anim pt-4 flex flex-wrap items-center gap-4 w-full">
              <button
                onClick={onOpenBooking}
                className="px-8 py-4 rounded-full font-bold text-[14px] md:text-[16px] uppercase tracking-wider text-[#0A0A0A] bg-[#00D28F] hover:bg-[#00B87D] shadow-lg shadow-[#00D28F]/25 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap"
              >
              ...
              <Link
                href="/portfolio"
                className="px-8 py-4 rounded-full font-bold text-[14px] md:text-[16px] uppercase tracking-wider text-[#1A1A1A] bg-zinc-100 hover:bg-zinc-200 border border-zinc-200 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 whitespace-nowrap"
              >
              ...
              >
                ...
                <div className="absolute bottom-4 left-4 right-4 z-10 p-4 rounded-xl bg-black/80 backdrop-blur-xl border border-white/15 text-white flex items-center justify-between shadow-2xl">
                  ...
                  <div className="text-right shrink-0">
                    <span className="text-[18px] md:text-[20px] font-mono font-bold text-[#00D28F]">
                      +210% CRO
                    </span>
                    <span className="text-[12px] text-zinc-400 block font-mono">Post Launch</span>
                  </div>
                </div>
              </div>
            </div>
